...
// Custom attention_forward using WMMA for batched matmuls
// Note: This uses kernels from llmc/attention.cuh (permute, softmax, unpermute)
// but replaces the batched matmul calls with our WMMA implementation
void attention_forward_wmma(floatX* out, floatX* qkvr, floatX* att,
    ...
    matmul_batched_wmma(preatt, k, q, nullptr, T, T, HS, B * NH, 
                       T * HS, T * HS, T * T, true, false, stream);
    
    // Scale and softmax
    float scale = 1.f / sqrtf(HS);
    int grid_size = CEIL_DIV(B * NH * T * WARP_SIZE, block_size);
    extern __global__ void softmax_forward_kernel5(floatX* out, float inv_temperature, 
